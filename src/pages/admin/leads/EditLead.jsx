...
function EditLead() {
  ...
  const fetchLead = async () => {
    try {
      ...
      setEmail(lead.email || "");
      setPhone(lead.phone || "");
      setLeadSource(lead.lead_source || "Other");
      setAssignedSalesperson(lead.assigned_salesperson || "");
      setStatus(lead.status || "New");
      setEstimatedDealValue(lead.estimated_deal_value || "");
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "Failed",
        text: error.response?.data?.message || "Failed to load lead details",
  ...
  const handleUpdateLead = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Update Failed",
        text: error.response?.data?.message || "Failed to update lead",
        confirmButtonColor: "#00a6fb",
      });
    } finally {
      setSaving(false);
